const { getEventController, searchById, searchByName } = require('../controllers/Event/getEventController')


const getEventHandler = async (req, res) => {
    const { name, eventType, country, date } = req.query;
    try {
        const events = await getEventController(name, eventType, country, date)
